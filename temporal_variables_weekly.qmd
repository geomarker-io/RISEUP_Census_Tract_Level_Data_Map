---
title: "Temporal Variables - Weekly"
format: html
editor: visual
execute: 
  echo: false
...
d_wide <- aqi |>
  ...
  mutate(across(AQI:`Relative Humidity`, ~zoo::na.approx(.x, na.rm = FALSE)),
         week = floor_date(date, "week")) |>
  group_by(week) |>
  summarize(across(.cols = AQI:`Relative Humidity`, ~mean(.x, na.rm = TRUE)))

ggpairs(d_wide, columns = 2:ncol(d_wide))
ggsave("plots/pollen_weather_scatter_weekly.png", width = 10, height = 8)
```
...
ggsave("plots/faceted_line_plot_week.png", height = 10)
```
...
ggsave("plots/smoothed_faceted_line_plot_week.png", height = 10)
```

```{r}
d_wide_log <- d_wide_log |>
  mutate(outdoor_mold_log = log(outdoor_mold))

d_wide_log <- select(d_wide_log, -pollen, -outdoor_mold)
